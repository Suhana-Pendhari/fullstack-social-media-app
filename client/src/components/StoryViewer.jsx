...
const StoryViewer = ({ viewStory, setViewStory }) => {
  ...
  const renderContent = ()=>{
    switch (viewStory.media_type) {
      case 'image':
        return (
          <img src={viewStory.media_url} alt='' className='w-full h-full object-contain' />
        );
      case 'video':
        return (
          <video onEnded={()=>setViewStory(null)} src={viewStory.media_url} className='w-full h-full object-contain' controls autoPlay />
        );
      case 'text':
        return (
  ...
  return (
    <div className='fixed inset-0 z-[9999] bg-black flex items-center justify-center' style={{ backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color : '#000000' }}>
